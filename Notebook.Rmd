...
percap_nonmetro <- nmcm %>%
  summarize(
    `2013` = sum(estimate_2013_total, na.rm = TRUE) / sum(population_2013, na.rm = TRUE),
    `2018` = sum(estimate_2018_total, na.rm = TRUE) / sum(population_2018, na.rm = TRUE),
    `2023` = sum(estimate_2023_total, na.rm = TRUE) / sum(population_2023, na.rm = TRUE)
  ) %>%
  pivot_longer(cols = everything(), names_to = "Year", values_to = "JobsPerCapita") %>%
